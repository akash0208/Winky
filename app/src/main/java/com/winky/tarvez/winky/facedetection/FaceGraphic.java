...
package com.winky.tarvez.winky.facedetection;
...
import com.winky.tarvez.winky.LivePreviewActivity;
import com.winky.tarvez.winky.common.GraphicOverlay;
...
public class FaceGraphic extends GraphicOverlay.Graphic {
    ...
    private static int countFR = 0;
    private static int countFL = 0;
    private static int countBR = 0;
    private static int countBL = 0;
    ...
    FaceGraphic(GraphicOverlay overlay, Context activity) {
        super(overlay);
...
//        idPaint);
        if (facing == CameraSource.CAMERA_FACING_FRONT) {
            if (face.getRightEyeOpenProbability() < 0.5) {
                countFR = countFR + 1;
                System.out.println("countR" + (countFR));
                if( countFR > 3) {
                    ((LivePreviewActivity) context).ankhmar();
                }
                else {
                    ((LivePreviewActivity) context).ankhmatmar();
                }
            }
            else if(face.getLeftEyeOpenProbability() < 0.5) {
                countFL = countFL + 1;
                System.out.println("countR" + (countFL));
                if( countFL > 3){
                    ((LivePreviewActivity) context).ankhmar();
                countFL = 0;
                } else {
